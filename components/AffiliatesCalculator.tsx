...
export default function AffiliatesCalculator({ data }: { data?: AffiliatesPageResult | null }) {
  const products =
    data?.calcProducts?.length && data.calcProducts.every((p) => p.id && p.name != null && p.price != null)
      ? data.calcProducts as { id: string; name: string; price: number; label: string }[]
      : DEFAULT_PRODUCTS;
  ...
  return (
    ...
    >
      ...
      <motion.div className="calc-copy" variants={sectionVariants}>
        <p className="calc-eyebrow">{data?.calcEyebrow ?? "// Your Potential Earnings"}</p>
        <h2 className="calc-heading">
          <em className="calc-heading-accent">
            {(data?.calcHeading ?? "See What Your Cut Looks Like.")
              .split("\n")
              .map((line, i) => (
