...
const DEFAULT_RESOURCES = [
  { id: "materials", title: "Marketing Materials", body: "Banners, copy blocks, and creative assets aligned with the WagerBird brand — ready to drop into any format.", href: "/register", highlighted: false },
  { id: "link", title: "Unique Tracking Link", body: "Your personal link tracks every click and purchase tied to your audience. Attribution is clear. No gaps.", href: "/register", highlighted: false },
  { id: "commission", title: "30% Flat Commission", body: "No tiers. No climbing. 30% on every first-time purchase from day one — Hotsheet or PointsPacks.", href: "/affiliates#calculator", highlighted: true },
  { id: "dashboard", title: "Partner Dashboard", body: "Real-time view of all clicks, conversions, and commissions. Full visibility — no delays, no batching.", href: "/register", highlighted: false },
  { id: "platform", title: "Platform Access", body: "Approved affiliates get direct access to WagerBird. Speak from experience, not theory. Your audience will feel the difference.", href: "/register", highlighted: false },
  { id: "support", title: "Dedicated Support", body: "Our team helps with creative strategy, campaign angles, and questions. You won't be figuring things out alone.", href: "/register", highlighted: false },
];
...
export default function PartnerResources({ data }: { data?: AffiliatesPageResult | null }) {
  const sectionRef = useRef<HTMLElement>(null);
  const inView = useInView(sectionRef, { once: true, margin: "-60px" });

  const eyebrow = data?.prEyebrow ?? "// Partner Resources";
  const heading1 = data?.prHeading1 ?? "Everything you need to";
  const heading2 = data?.prHeading2 ?? "Start Promoting.";
  const description =
    data?.prDescription ??
    "You handle the audience. We handle the assets. Approved affiliates can ship on day one.";
  const resources =
    data?.prResources?.length && data.prResources.every((r) => r.id && r.title && r.body)
      ? (data.prResources as { id: string; title: string; body: string; href?: string; highlighted?: boolean }[])
      : DEFAULT_RESOURCES;
    ...
    <section ref={sectionRef} className="pr-section">
      ...
      >
        ...
        <h2 className="pr-heading">
          <span className="pr-heading-white">{heading1}</span>
          <br />
          <span className="pr-heading-yellow">{heading2}</span>
        </h2>
        <p className="pr-description">{description}</p>
      </motion.div>
      ...
      >
        {resources.map((resource) => (
          ...
          >
            <Link href={resource.href ?? "/register"} className="pr-card">
              {/* Icon box */}
              <div className="pr-icon-box">
                {ICON_MAP[resource.id] ?? ICON_MAP["materials"]}
              </div>
